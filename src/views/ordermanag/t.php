...
class ContentController extends Controller
{
    ...
    public function mod()
    {
        ...
        if ($_POST) {
            ...
            if($content){
                $contentimgs=$this->content_filter($content);
                foreach ($contentimgs as $kc=>$vc){
                    $ishttp=substr($vc,0, 4);
                    $ishttps=substr($vc,0, 5);
                    if($ishttp=="http"||$ishttps=="https"){//是远程图片--下载
                        $content_img = file_get_contents($vc); // 得到二进制图片内容
                        $url='upload/image/'.date('Ymd',time()).'/'.$this->getfilename().'.jpg';
                        $path = ROOT_PATH . 'static' . DS . $url;
                        file_put_contents($path, $content_img);
                        $content=str_replace($vc,'/'.$url,$content);
                    }
                }
            }
            // 构建数据
    ...
    /**
     * [content_filter 文章内容过滤 ]
     */
    public function content_filter($str){
        preg_match_all('/<img[^>]*src\s*=\s*([\'"]?)([^\'" >]*)\1/isu', $str, $src);
        return $src[2];
    }
    /**
     * [content_filter 文章内容过滤 data-src ]
     */
    public function content_filters($str){
        preg_match_all('/<img[^>]*data-src\s*=\s*([\'"]?)([^\'" >]*)\1/isu', $str, $src);
        return $src[2];
    }
}